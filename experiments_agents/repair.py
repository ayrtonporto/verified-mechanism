...
class TargetedRepairAgent:
    ...
    def __init__(
        ...
        *,
        ...
        solution_guard: Callable[[Problem, Services, str], Awaitable[Any]] | None = None,
        ...
        self.solution_guard = solution_guard
    ...
    async def solve(self, problem: Problem, services: Services) -> AgentResult:
        ...
        candidate = self.initial_candidate or challenge
        ...
        plan = ([] if self.initial_candidate is not None else [
        ...
        parent_hash = sha16(candidate)
        if self.initial_candidate is not None:
            seed_check = await services.lean.check_file(candidate)
            lean_checks += 1
            seed_assessment = self._assess(candidate, True, challenge, seed_check)
            seed_guard_ok = True
            if (
                seed_check.accepted
                and seed_assessment.integrity_ok
                and self.solution_guard is not None
            ):
                try:
                    guarded = await self.solution_guard(problem, services, candidate)
                    seed_guard_ok = bool(getattr(guarded, "accepted", guarded))
                    guard_errors = list(getattr(guarded, "errors", ()) or ())
                except Exception as exc:
                    seed_guard_ok = False
                    guard_errors = [f"candidate guard failed closed: {exc}"]
                if not seed_guard_ok:
                    guard_rejections += 1
                    seed_assessment.integrity_ok = False
                    seed_assessment.integrity_errors.extend(
                        str(error) for error in guard_errors
                    )
                    semantic = "\n".join(str(error) for error in guard_errors)
                    seed_assessment.diagnostics = (
                        seed_assessment.diagnostics
                        + "\n\nSemantic candidate rejection:\n"
                        + semantic
                    ).strip()
                    seed_assessment.rank[0] = 0
                    seed_assessment.rank[1] = 0
            best_rank = seed_assessment.rank
            last_diagnostics = seed_assessment.diagnostics
            last_integrity_errors = seed_assessment.integrity_errors
            if (
                seed_check.accepted
                and seed_assessment.integrity_ok
                and seed_guard_ok
                ...
                return self._result(
                    ...
                    stop_reason="accepted_seed",
                    best_hash=sha16(candidate),
        ...
        for stage, model, turn, is_last in plan:
            ...
            guard_ok = True
            if check.accepted and a.integrity_ok and self.solution_guard is not None:
                try:
                    guarded = await self.solution_guard(problem, services, candidate)
                    guard_ok = bool(getattr(guarded, "accepted", guarded))
                    guard_errors = list(getattr(guarded, "errors", ()) or ())
                except Exception as exc:
                    guard_ok = False
                    guard_errors = [f"candidate guard failed closed: {exc}"]
                if not guard_ok:
                    ...
                    a.integrity_errors.extend(str(error) for error in guard_errors)
                    semantic = "\n".join(str(error) for error in guard_errors)
                    a.diagnostics = (
                        a.diagnostics + "\n\nSemantic candidate rejection:\n" + semantic
                    ).strip()
                    # A tautological REPL pass is not a substantive accepted candidate.
                    a.rank[0] = 0
                    a.rank[1] = 0
            attempts.append(
                Attempt(
                    ...
                    turn=turn,
                    model=model,
                    accepted=bool(check.accepted and guard_ok),
                    timed_out=check.timed_out,
                    message_count=len(check.messages),
            ...
            if check.accepted and a.integrity_ok and guard_ok:
                return self._result(
                    candidate,
                    accepted=True,
                    attempts=attempts,
        ...
        *,
        ...
        return AgentResult(
            solution,
            {
                ...
                "best_hash": best_hash,
                "guard_rejections": guard_rejections,
                "seeded": self.initial_candidate is not None,
                "calls_q": calls_q,
                "calls_g": calls_g,
                "lean_checks": lean_checks,
