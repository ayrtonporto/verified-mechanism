...
_DECL_SIGNATURE_RE = re.compile(
    r"(?m)^[ \t]*(?:(?:noncomputable|private|protected|scoped)\s+)*"
...
def declaration_signatures(source: str) -> dict[str, str]:
    """Return whitespace-normalized, proof-free declaration signatures.

    The assignment comparator locks theorem statements and declaration kinds.  Merely
    retaining a declaration *name* is insufficient: a model can unfold an answer in a
    theorem statement (or otherwise weaken it) while still producing a file accepted by
    the warm REPL.  For the challenge format, the first top-level ``:=`` separates the
    immutable signature from the replaceable value/proof.  Keeping the delimiter in the
    signature also distinguishes malformed declarations which omit it.

    This is deliberately textual and conservative.  The agent owns the original header,
    so harmless model reformatting is allowed only up to whitespace normalization; a
    semantic rewrite must be grafted back under the original header before acceptance.
    """

    text = source or ""
    matches = list(_DECL_SIGNATURE_RE.finditer(text))
    signatures: dict[str, str] = {}
    for index, match in enumerate(matches):
        end = matches[index + 1].start() if index + 1 < len(matches) else len(text)
        block = text[match.start():end]
        delimiter = block.find(":=")
        if delimiter < 0:
            signature = block
        else:
            signature = block[: delimiter + 2]
        signatures[match.group(2)] = re.sub(r"\s+", " ", signature).strip()
    return signatures
...
def integrity_check(candidate: str, challenge: str) -> tuple[bool, list[str]]:
    ...
    *original* challenge. Conservatively require every declaration named in the
    challenge to still be declared, and reject forbidden escapes.  This intentionally
    remains a permissive *inner-search* check: dependent slot candidates are later
    grafted under machine-owned headers before strict acceptance.
    """
...
def strict_integrity_check(candidate: str, challenge: str) -> tuple[bool, list[str]]:
    """Final candidate guard: inner hygiene plus immutable declaration signatures."""

    ok, errors = integrity_check(candidate, challenge)
    errors = list(errors)
    candidate_signatures = declaration_signatures(candidate)
    challenge_signatures = declaration_signatures(challenge)
    for name in required_decl_names(challenge):
        if name in candidate_signatures and (
            candidate_signatures[name] != challenge_signatures.get(name)
        ):
            errors.append(f"changed required declaration signature `{name}`")
